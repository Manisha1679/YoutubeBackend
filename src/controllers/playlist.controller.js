...
const deletePlaylist = asyncHandler(async(req,res)=>{
  ...
  return res
         .status(200)
         .json(
            new ApiResponse(200,{},"Playlsit deleted successfully")
         )
});
...
const getPlaylistById = asyncHandler(async (req,res) => {
  ...
  },
  {
    $lookup: {
      from: "videos",
      localField: "videos",     // This is an array of video IDs in the playlist
      foreignField: "_id",      // Match them with actual videos
      as: "videos"               // Result: full video objects added to "videos" array
    }
  },
  {
    $addFields: {
      videos: {
        $filter: {
          input: "$videos",     // the videos array from previous step
          as: "video",           // like forEach video in videos
          cond: { $eq: ["$$video.isPublished", true] }   // keep only if published
        }
      }
    }
  },
  ...
  },
  {
    $addFields: {
      ...
      owner: { 
        $first: "$owner"     // Turn array into single object
    }
    }
  },
  {
...
//Syntax	          Meaning	                                    Used Where
//videos	Field name (without context)	                      Top-level or JSON
//$videos	Reference to a field inside an operator         	  $project, $addFields, etc.
//$$video	Reference to a loop variable	                      Inside $filter, $map, etc.